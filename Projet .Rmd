...
Frequencies=variable_selection(Y,X,square_root_inv_hat_Sigma,nb_repli=50,parallel=FALSE,nb.cores=1)   
```
```{r}
...
data_genes_kept=Frequencies[Frequencies$frequency>=1,] # 452 lignes 
```
```{r}
...
theme_bw()+ylab('Dose_Temps_execution')+xlab('Gene')
p
...
```{r}
indices = which(Frequencies$frequency>=1)
...
```{r}
gr = ggplot(data=Frequencies[Frequencies$frequency>=1,],
